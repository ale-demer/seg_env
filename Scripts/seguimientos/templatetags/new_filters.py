...
def points_sum(user):
    ...
    ventas = VentaNueva.objects.filter( owner=user.id, payoff_date__month=current_month )
    i = 0
    for venta in ventas:
...
def total_daily_sales_by_vendor(user):
    ...
    for v in range(len(vendor_name)):
        for vendor in vendor_name[v]:
            ventas = VentaNueva.objects.filter( date_added__day=current_day , owner__username=vendor )
            i = 0
            for venta in ventas:
                i += 1
                vendor_name[v][1] = i
